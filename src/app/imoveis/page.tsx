...
import { Building2, LayoutGrid, List, SlidersHorizontal, Sparkles, X, ChevronDown } from 'lucide-react';
  ...
  return (
    <div className="pt-28 pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
      ...
      <div className="space-y-2 text-center sm:text-left">
        ...
        <p className="text-slate-200 text-base max-w-2xl font-normal">
      ...
      {activeFilterTags.length > 0 && (
        <div className="flex flex-wrap items-center gap-2 pt-2">
          <span className="text-xs text-slate-400 font-semibold mr-1">Filtros Ativos:</span>
          {activeFilterTags.map((tag) => (
            <span
              key={tag.key}
              className="inline-flex items-center gap-1.5 bg-gold-500/10 border border-gold-500/30 text-gold-300 text-xs px-3 py-1 rounded-lg"
            >
              <span>{tag.label}</span>
              <button
                onClick={() => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-4 border-t border-slate-800">
        <div className="text-xs text-slate-300 font-medium">
          Exibindo <span className="font-bold text-gold-400">{filteredProperties.length}</span> imóvei(s) encontrado(s)
        </div>

        <div className="flex items-center justify-between sm:justify-end gap-4">
          {/* Sorting */}
          <div className="flex items-center gap-2">
            <span className="text-xs text-slate-400 font-medium hidden sm:inline">Ordenar por:</span>
            <select
              value={sortOption}
              onChange={(e) => handleSortChange(e.target.value)}
              className="bg-slate-900 border border-slate-700/80 rounded-xl px-3 py-2 text-white text-xs focus:outline-none focus:border-gold-500"
            >
          ...
          <div className="flex items-center gap-1 bg-slate-900 p-1 rounded-xl border border-slate-800">
            <button
              ...
              className={`p-1.5 rounded-lg transition-colors ${
                viewMode === 'grid' ? 'bg-gold-500 text-navy-950' : 'text-slate-400 hover:text-white'
              }`}
            ...
            <button
              ...
              className={`p-1.5 rounded-lg transition-colors ${
                viewMode === 'list' ? 'bg-gold-500 text-navy-950' : 'text-slate-400 hover:text-white'
              }`}
      ...
      {filteredProperties.length > 0 ? (
        ...
        <div className="glass-card p-16 rounded-3xl text-center space-y-4 max-w-2xl mx-auto border border-slate-800">
          <Building2 className="w-12 h-12 text-gold-500/40 mx-auto" />
          ...
          <button
            onClick={() => handleFilterChange({})}
            className="px-6 py-3 rounded-xl bg-gold-gradient text-navy-950 font-bold text-xs uppercase tracking-wider hover:brightness-110 transition-all shadow-glow-gold"
          >
